...
;; 2. Description: M_DEP_AA_Neb_at_Insp_15cm
;; x1. Author: user

$SIZES DIMNEW=-5000
$PROBLEM Semi-Mechanistic Model for Aerosol Delivery in Mechanical Ventilation
; No Humidifier Dry End or Humidifier; Nebulization at Inspiratory Limb 15cm from Y-piece
...
F_INSP = THETA(14) ; 吸气支界面沉积常数 (类似于旧 F_DRY)
W_DEP_Y = 12
W_DEP_EXT = 22
W_DEP_AA = 60
W_DEP_EXP = 12
...
; ───────────────────────────────────────────────
; 1. 吸气支 (Inspiratory Limb) ─ 喷射撞击沉积 (类似于旧干端)
; ───────────────────────────────────────────────
QM_INSP = QM_INSP
...
; ───────────────────────────────────────────────
; 4. Artificial Airway (气管导管) ─ 根据相位动态计算
; ───────────────────────────────────────────────
IF (IT .EQ. 1) THEN
RR0 = 0.5
ELSE
...
; ───────────────────────────────────────────────
; 5. 呼气肢 (Expiratory Limb) ─ 三段直管 + 两弯（向下）
; ───────────────────────────────────────────────
...
NOPRINT ONEHEADER FILE=sim_9-2.tab